...
public class FilterToolbox extends VizPanel implements TouchEnabled,
	EventSubscriber {
    public FilterToolbox(float x0, float y0, float width, float height,
	    VizPanel parent) {
    ...
    private VizButton weatherButton, alcoholButton, speedButton, vehicleButton,
	    ageButton, dayButton, monthButton, hourButton, sexButton, lcButton,
	    applyButton, removeButton;
	...
	if (down) {
		...
		selector = 0;
		NotificationCenter.getInstance().notifyEvent("update-selector",
			0);
		...
		NotificationCenter.getInstance().notifyEvent("remove-graph",
			selector);
	    }
	...
	// Remove Button
	removeButton = new VizButton(REMOVE_X0, REMOVE_Y0, REMOVE_W, REMOVE_Y,
		this);
	...
	// Days of the week
	dayList = new VizList(SECOND_COL_X0, THIRD_ROW_Y0 + BUTTON_H, LIST_W,
		LIST_H, this);
	dayList.setup(LIGHT_GRAY, DARK_GRAY, N_ROWS, getDaysOfWeek(), false,
		SelectionMode.MULTIPLE);
	dayList.setListName(DAY_OF_WEEK);

	dayButton = new VizButton(SECOND_COL_X0, THIRD_ROW_Y0, LIST_W,
		BUTTON_H, this);
	dayButton.setText("Day of Week");
	listsAndButtons.put(dayButton, dayList);

	// Weather Condition
	weatherList = new VizList(FIRST_COL_X0, FIRST_ROW_Y0 + BUTTON_H,
		2 * LIST_W + 25, LIST_H, this);
	weatherList.setup(LIGHT_GRAY, DARK_GRAY, N_ROWS,
		getWeatherConditions(), false, SelectionMode.MULTIPLE);
	weatherList.setListName(WEATHER_CONDITION);

	weatherButton = new VizButton(FIRST_COL_X0, FIRST_ROW_Y0, LIST_W,
		BUTTON_H, this);
	weatherButton.setText("Weather");
	listsAndButtons.put(weatherButton, weatherList);

	// Months
	monthList = new VizList(FIRST_COL_X0, FOURTH_ROW_Y0 + BUTTON_H, LIST_W,
		LIST_H, this);
	monthList.setup(LIGHT_GRAY, DARK_GRAY, N_ROWS, getMonths(), false,
		SelectionMode.MULTIPLE);
	monthList.setListName(MONTHS);

	monthButton = new VizButton(FIRST_COL_X0, FOURTH_ROW_Y0, LIST_W,
		BUTTON_H, this);
	...
	// Type of Vehicle
	vehicleList = new VizList(SECOND_COL_X0, SECOND_ROW_Y0 + BUTTON_H,
		LIST_W + 25, 90, this);
	vehicleList.setup(LIGHT_GRAY, DARK_GRAY, 4, getVehicles(), false,
		SelectionMode.MULTIPLE);
	vehicleList.setListName(VEHICLE);

	vehicleButton = new VizButton(SECOND_COL_X0, SECOND_ROW_Y0, LIST_W,
		BUTTON_H, this);
	...
	// Hour
	hourList = new VizList(SECOND_COL_X0, FOURTH_ROW_Y0 + BUTTON_H, 70,
		LIST_H, this);
	hourList.setup(LIGHT_GRAY, DARK_GRAY, N_ROWS, getHour(), false,
		SelectionMode.MULTIPLE);
	hourList.setListName(HOUR);

	hourButton = new VizButton(SECOND_COL_X0, FOURTH_ROW_Y0, LIST_W,
		BUTTON_H, this);
	hourButton.setText("Hour");
	listsAndButtons.put(hourButton, hourList);

	// Sex
	sexList = new VizList(FIRST_COL_X0, FIFTH_ROW_Y0 + BUTTON_H, LIST_W,
		40, this);
	sexList.setup(LIGHT_GRAY, DARK_GRAY, 2, getSex(), false,
		SelectionMode.MULTIPLE);
	sexList.setListName(SEX);

	sexButton = new VizButton(FIRST_COL_X0, FIFTH_ROW_Y0, LIST_W, BUTTON_H,
		this);
	sexButton.setText("Sex");
	listsAndButtons.put(sexButton, sexList);

	// Light Condition
	lcList = new VizList(SECOND_COL_X0, FIFTH_ROW_Y0 + BUTTON_H,
		LIST_W + 20, LIST_H, this);
	lcList.setup(LIGHT_GRAY, DARK_GRAY, N_ROWS, getLightConditions(),
		false, SelectionMode.MULTIPLE);
	lcList.setListName(LIGHT_CONDITION);

	lcButton = new VizButton(SECOND_COL_X0, FIFTH_ROW_Y0, LIST_W, BUTTON_H,
		this);
	...
	for (Map.Entry<VizButton, VizList> entry : listsAndButtons.entrySet()) {
	    entry.getKey().setStyle(MEDIUM_GRAY, WHITE, DARK_WHITE, 255, 255,
		    14);
	    entry.getKey().setStyleDisabled(MEDIUM_GRAY, WHITE, DARK_WHITE,
		    255, 255, 14);
	...
	text("State: " + currentState.getName(), 30, 35);
	popStyle();
    }
	...
	fill(LIGHT_GRAY);
	rect(TABBUTTON_W / 2, 0, getWidth() - TABBUTTON_W / 2, getHeight(), 5,
		5, 5, 5);
