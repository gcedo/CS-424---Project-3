...
public class VizMap extends VizPanel implements TouchEnabled, EventSubscriber {
	...
	private void drawLocationMarkers() {
		if (map.sc>CLUSTER_SC_MAX) {
			...
			for (AbstractMarker a : markers) {
				...
				// if it is in the viewable area of the map, we try to cluster it
				...
				if (clusters.size() == 0) {
					...
					// number of it as 1 and set the position of it as the position
					// of a
					clusters.add(cl);
				}
				else {
					...
					for (Cluster cl : clusters) {
						...
						if (dis<10000) {
							...
							// TODO if the position of this cluster changes, that'll
	...
	private float dist(AbstractMarker a, Cluster cl) {
		return(
				(a.getX0() - cl.getPos().x) * (a.getX0() - cl.getPos().x)
				+ (a.getY0() - cl.getPos().y) * (a.getY0() - cl.getPos().y) 
			);
	}
