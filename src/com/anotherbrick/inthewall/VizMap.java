...
public class VizMap extends VizPanel implements TouchEnabled, EventSubscriber {
  ...
  /*
   * private void drawLocationMarkers() { //right is larger than left float
   * lon_right = getP().map((float)(-map.tx + getWidth()*0.5/map.sc),
   * (float)0.0,(float)256.0,(float)-180.0,(float)180.0); float lon_left =
   * getP().map((float)(-map.tx - getWidth()*0.5/map.sc),
   * (float)0.0,(float)256.0,(float)-180.0,(float)180.0); //top is larger than
   * bot float lat_top = getP().map((float)(-map.ty - getHeight()*0.5/map.sc),
   * (float)0.0,(float)256.0,(float)90.0,(float)-90.0); float lat_bot =
   * getP().map((float)(-map.ty + getHeight()*0.5/map.sc),
   * (float)0.0,(float)256.0,(float)90.0,(float)-90.0); for (AbstractMarker a :
   * markers) { //if ((-map.tx+getWidth()*0.5/map.sc) && a.getX0() > -map.tx -
   * 537 * 0.5 / map.sc && a.getY0() < -map.ty + 384 * 0.5 / map.sc && a.getY0()
   * > -map.ty - 384*0.5/map.sc) { if (a.getX0() < lon_right && a.getX0() >
   * lon_left) { //&& a.getY0() < lat_top && a.getY0() > lat_bot) { a.draw();
   * System.out.println("drawn!"); } else { System.out./("didn't!"); } }
   * //System
   * .out.println(lon_left+" "+(-map.tx)+" "+lon_right+";  \t"+lat_top+" "
   * +(-map.ty)+" "+lat_bot);
   * 
   * }
   */
  ...
  private void updateLocationMarkers() {
    ...
    for (Map.Entry<Integer, LocationWrapper> pair : locationsList.entrySet()) {
      ...
      AbstractMarker marker = wrapper.getCorrespondingMarker(point.x, point.y, MARKER_WIDTH,
  ...
  private void setYear(Integer year) {
    locationsList.clear();
    addLocations(DBUtil.getInstance().getPointsByState(selectedStateId, year));
  }
