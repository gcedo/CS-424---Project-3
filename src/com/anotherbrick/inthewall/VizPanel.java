...
public abstract class VizPanel {
    ...
    // }
    //
    // public static VizPanel createFromXYandDimensions(float x0, float y0,
    // float
    // width, float height) {
    ...
    private float x0Zoom, y0Zoom, widthZoom, heightZoom, parentX0Zoom,
	    parentY0Zoom;
    ...
    private VizPanel(float x0, float y0, float width, float height,
    ...
    public VizPanel(float x0, float y0, float width, float height,
	    VizPanel parent) {
	this(x0, y0, width, height, parent.getX0Absolute(), parent
		.getY0Absolute());
	this.parent = parent;
    }
    ...
    public void arc(float x, float y, float startEdge, float stopEdge,
	    float startAngle, float stopAngle) {
	p.arc(x0Zoom + s(x), y0Zoom + s(y), s(startEdge), s(stopEdge),
    ...
    public void background(MyColorEnum color, float alpha, float tl, float tr,
	    float br, float bl) {
	pushStyle();
	fill(c.myColor(color), alpha);
	p.rect(x0Zoom, y0Zoom, widthZoom, heightZoom, s(tl), s(tr), s(br),
		s(bl));
	popStyle();
    }
	...
	return x > x0Zoom && x < x0Zoom + widthZoom && y > y0Zoom
		&& y < y0Zoom + heightZoom;
    }
	...
	if (isPg) {
	    ...
	    p.line(x0Zoom + s(x1), y0Zoom + s(y1), x0Zoom + s(x2), y0Zoom
		    + s(y2));
	}
    }
	...
	println("[" + dateFormat.format(date)
		+ String.format("%25s", this.getClass().getSimpleName()) + "] "
		+ msg);
    }
    ...
    public void modifyPositionAndSize(float newX0, float newY0, float newWidth,
	    float newHeight) {
	modifyPosition(newX0, newY0);
	modifySize(newWidth, newHeight);
    }

    public void modifyPositionAndSizeWithAbsoluteValue(float newX0,
	    float newY0, float newWidth, float newHeight) {
    ...
    public void rect(float a, float b, float c, float d, float tl, float tr,
	    float br, float bl) {
	p.rect(x0Zoom + s(a), y0Zoom + s(b), s(c), s(d), s(tl), s(tr), s(br),
		s(bl));
    }
