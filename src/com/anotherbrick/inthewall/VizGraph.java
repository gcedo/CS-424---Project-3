...
public class VizGraph extends VizPanel implements TouchEnabled, EventSubscriber {
	...
	yearSlider = new YearSlider(PLOT_PADDING_LEFT - HALF_SLIDER,
		PLOT_PADDING_TOP, SLIDER_HEIGHT, this);
	...
	event = new HistoricalEvent(getWidth() / 2 - 100, getHeight() / 2 - 50,
		200, 100, this);

	NotificationCenter.getInstance().registerToEvent("update-graph", this);
	NotificationCenter.getInstance().registerToEvent("update-selector",
		this);
	...
	line(PLOT_PADDING_LEFT, getHeight() - PLOT_PADDING_BOTTOM, getWidth()
		- PLOT_PADDING_RIGHT, getHeight() - PLOT_PADDING_BOTTOM);
	line(PLOT_PADDING_LEFT, PLOT_PADDING_TOP, PLOT_PADDING_LEFT,
	...
	if (eventName.equals("update-graph")) {
	    PlotData pdata = new PlotData((ArrayList<PVector>) data,
		    palette[selector]);
	...
	if (plot != null) {
		...
		float x = PApplet.map(
			...
			xStop, PLOT_PADDING_LEFT, getWidth()
				- PLOT_PADDING_RIGHT);
		float y = PApplet.map(
			((PVector) p[i - (int) plot.getXMin()]).y, 0,
			getOverallYMax(plots), getHeight()
				- PLOT_PADDING_BOTTOM, PLOT_PADDING_TOP);
		if (!clustered) {
		    vertex(x, y);
	    ...
	    if (plot.isFilled()) {
		vertex(getWidth() - PLOT_PADDING_RIGHT, getHeight()
			- PLOT_PADDING_BOTTOM);
	...
	noStroke();
	rect(PLOT_PADDING_LEFT, PLOT_PADDING_TOP, getWidth()
		- PLOT_PADDING_LEFT - PLOT_PADDING_RIGHT, getHeight()
		- PLOT_PADDING_BOTTOM - PLOT_PADDING_TOP);
	popStyle();
    }
	...
	for (int i = (int) xStart; i <= xStop; i++) {
	    int x = (int) PApplet.map(i, xStart, xStop, PLOT_PADDING_LEFT,
		    getWidth() - PLOT_PADDING_RIGHT);
	    fill(MyColorEnum.WHITE);
	    switch (i) {
	    case 2003:
	...
	for (int i = 0; i < getOverallYMax(plots); i += range) {
	    int y = (int) PApplet.map(i, 0, getOverallYMax(plots), getHeight()
		    - PLOT_PADDING_BOTTOM, PLOT_PADDING_TOP);
	...
	if (yearSlider.isMoving())
	    yearSlider.modifyPositionWithAbsoluteValue(
		    costrain(m.touchX, getX0Absolute() + getWidth()
			    - HALF_SLIDER - PLOT_PADDING_RIGHT,
			    PLOT_PADDING_LEFT - HALF_SLIDER + getX0Absolute()),
		    yearSlider.getY0Absolute());
